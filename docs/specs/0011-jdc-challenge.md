...
- [ ] A visit containing a single, a double and a treble of the round's number
      scores the three darts' face value **plus 100**. S15/D15/T15 scores 190.
- [ ] A visit without all three rings scores face value only, unchanged.
- [ ] Part 2 is unchanged: 50 per double, 100 for the inner bull, 0 for the
      outer 25 and for misses.
- [ ] The challenge still completes at exactly 57 darts and ignores darts
      after that.
- [ ] `lib/content/guides.ts` describes the additive bonus, and the tip no
      longer advises against Shanghais.
- [ ] Every past attempt's `results.metrics.gameScore` is recomputed to the
      corrected scale, so no chart mixes the two.

### The front door

- [ ] `/jdc` exists and is reachable from `/training` and from `/practice`.
- [ ] `/jdc` has a start button that creates a `jdc-challenge` game and opens
      the normal pad.
- [ ] The challenge remains absent from the `/practice` drill grid: it is a
      test, not a drill, and spec 0005's eight-game scope rule stands.
- [ ] With no attempts, `/jdc` states the action: "No attempts yet. Throw the
      challenge."

### The progress record

- [ ] `/jdc` shows the current grade, the best score, the latest score, and
      the change on the previous attempt.
- [ ] `/jdc` shows the distance to the next grade in points.
- [ ] `/jdc` charts every attempt in order with the grade bands drawn behind
      it.
- [ ] Attempts thrown as a programme assessment are marked distinctly from
      ad-hoc attempts.
- [ ] `/jdc` breaks the latest attempt into part 1, part 2 and part 3 points,
      each against its own personal best.
- [ ] Tapping an attempt opens its detail: part totals, Shanghais hit, and
      which doubles were hit and missed in part 2.
- [ ] Every figure on `/jdc` is derived from the dart log, not stored
      (ADR 0003).

### Mid-challenge

- [ ] The live band shows the running points against the same point in the
      best attempt ("PB pace +24" / "−15"), when a best attempt exists.
- [ ] Parts 1 and 3 show S/D/T pips that light as rings land and reset each
      round, matching the Shanghai drill's existing pattern.
- [ ] Part 2 shows doubles hit out of 21.
- [ ] The progress bar reads as three parts rather than one run of 57.
...
```ts
{
  ...
  best: { score: number; grade: string; achievedAt: string } | null
  partBests: [number, number, number]
  grade: { name: string; since: string | null; next: { name: string; pointsAway: number } | null }
}
```
...
Returns the single attempt above plus `doubles: Array<{ target: number; hit: boolean }>`
for the part-2 board, and `rounds` for parts 1 and 3.

Both are served by `lib/server/jdc.ts` and mirrored by an MSW handler in
`mocks/handlers/jdc.ts`, per Rule 5.
...
Top to bottom, in the app's existing idiom (dark, sentence case, no
congratulation):

```
JDC CHALLENGE                                    4 attempts

  GREEN                                          Best 488
  488 pts                                        +36 on last

  Blue at 600 — 112 points away
  ────────────────────────────────────────────────

  [ trend chart: one bar per attempt, grade bands as
    horizontal rules behind, programme assessments
    marked with a tick under the bar ]

  THIS ATTEMPT                    latest / best
  Part 1  Shanghai 10-15     142 / 168  ▓▓▓▓▓▓░░░
  Part 2  Doubles            250 / 300  ▓▓▓▓▓▓▓░░
  Part 3  Shanghai 15-20      96 / 151  ▓▓▓▓░░░░░

  ATTEMPTS
  Green   488 pts · 12 Sep · assessment
  Yellow  452 pts · 29 Aug · assessment
  ...

  [ Throw the challenge ]
```

- `/components/jdc/GradeBadge.tsx` — grade name in its JDC colour. The seven
  colours are the JDC's own device; do not invent a parallel one.
- `/components/jdc/TrendChart.tsx` — bars, not a line: attempts are discrete
  fortnightly events, not a continuous series. Grade bands behind as thin
  rules with their names at the left edge. No axis furniture.
- `/components/jdc/PartBars.tsx` — three rows, latest against best.
- `/components/jdc/AttemptList.tsx` — reuses the existing `/training` row
  styling so the two screens read as one product.
...
`mocks/data/seed.json` gains **five completed JDC attempts** on a rising
trend across three grades, with full 57-dart logs, and the training sessions
that two of them fulfilled. Without them the hub has never been seen with data
in it. Generated by `scripts/generate-seed.ts`, deterministically, like
everything else.
...
These change the build and are Tom's to settle. The recommendation is given
first in each case.

1. **Does the displayed grade follow the best attempt or the latest?**
   *Recommend best.* A grade is something you reached; one bad night should
   not take it off you. The trend chart carries the honesty.
2. **Do ad-hoc attempts count toward the trend, or only programme
   assessments?** *Recommend all of them, marked.* Throwing it twice in a
   week is a legitimate thing to do; hiding it is worse than labelling it.
3. **Outer bull in part 2.** Currently scores nothing, which is consistent
   with "only the double counts" — D25 is the double of 25 and the outer ring
   is not. *Recommend keeping it.* Flagged because no source states it
   explicitly.
4. **Backfill or reset after the scoring fix?** *Recommend backfill* — the
   logs are intact, so rescoring is exact, and the alternative throws away a
   real record for no gain.
...
| Slice | Contents | Size |
|---|---|---|
| **A — correctness** | The one-line engine fix, its test, the guide copy and tip, the `gameScore` backfill | small |
| **B — front door and record** | `lib/practice/games/jdc-report.ts`, `GET /api/jdc`, the MSW handler, `/jdc` hub with grade, trend and attempt list, entry points from `/training` and `/practice`, seed attempts | medium |
| **C — depth** | Part bars and per-part bests, attempt detail screen, PB-pace and pips in the live band | medium |

Slice A should land on its own and first. Every number slice B draws depends
on it being right, and it is the only part of this spec that changes what a
score *means*.
